...
class XlsxPropertyReport(http.Controller):
    ...
    @http.route(
        "/property/excel/report/<string:property_ids>", type="http", auth="user"
    )
    def download_property_excel_report(self, property_ids):
        ...
        property_ids = request.env["property"].browse(
            literal_eval(property_ids)
        ...
        header_format = workbook.add_format(
            {"bold": True, "bg_color": "gray", "border": 1, "align": "center"}
        )
        string_format = workbook.add_format({"border": 1, "align": "center"})
        price_format = workbook.add_format(
            {"num_format": "$##,##00.00", "border": 1, "align": "center"}
        )
        ...
        for col_num, header in enumerate(headers):
            ...
            )  # (row, cloumn, Value_in_cell, format)
        ...
        for property in property_ids:
            ...
            worksheet.write(
                row_num, 3, "YES" if property.garden else "NO", string_format
            )
            row_num += 1
