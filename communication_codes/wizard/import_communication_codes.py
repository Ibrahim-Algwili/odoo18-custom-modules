...
class ImportCommunicationCodesWizard(models.TransientModel):
    ...
    def _process_row(self, row, row_idx):
        ...
        code_version = self._map_value(
            row[9], self.CODE_VERSION_MAP, "version", row_idx
        )
        ...
        if existing_code:
            ...
            if self.import_mode == "update":
                return {
                    "error": f'Row {row_idx}: Code "{code_number}" not found for update.'
                }
    ...
    def _find_existing_code(self, code_number):
        return self.env["communication.codes"].search(
            [("code_number", "=", code_number)], limit=1
        )
    ...
    def _map_value(self, value, mapping, field_type, row_idx):
        ...
        if not mapped_value:
            _logger.warning(f'Row {row_idx}: Invalid {field_type} value "{value}"')
            defaults = {
                "system": "prepaid",
                "status": "in_stock",
                "version": "original",
