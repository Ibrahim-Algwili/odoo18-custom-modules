...
class CommunicationCodes(models.Model):
        ...
        for record in self:
            if record.code_status == 'delivered':
                if not record.delivery_date or not record.delivery_user_id:
                    raise ValidationError(
                        _('Delivery date and delivered by user are required when status is Delivered.')
                    )
    ...
    def get_dashboard_stats(self):
        ...
        status_data = self.read_group(
            domain, ['code_status'], ['code_status']
        )
        ...
        system_data = self.read_group(
            domain, ['code_system'], ['code_system']
        )
        ...
        for comp in company_group:
            ...
            company_name = comp.get('company_id')[1] if comp.get('company_id') else _('Unknown')
